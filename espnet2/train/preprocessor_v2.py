...
from espnet2.text.build_tokenizer_v2 import build_tokenizer
...
class S2TPreprocessor(CommonPreprocessor):
    def __init__(
        ...
        utt_time_apply_prob: float = 0.5,
        word_time_apply_prob: float = 0.5,
        notime_symbol: str = "<notimestamps>",
        word_time_symbol: str = "<wordtimestamps>",
        utt_time_symbol: str = "<utttimestamps>",
        ...
        self.text_word_name = text_word_name
        ...
        self.utt_time_apply_prob = utt_time_apply_prob
        self.word_time_apply_prob = word_time_apply_prob

        # Obtain the token id of special tokens
        self.na_symbol = na_symbol
        self.na_idx = self.token_id_converter.token2id[na_symbol]
        self.notime = self.token_id_converter.token2id[notime_symbol]
        self.word_time_symbol = self.token_id_converter.token2id[word_time_symbol]
        self.utt_time_symbol = self.token_id_converter.token2id[utt_time_symbol]
    ...
    def _text_process(
        ...
        text_names = [self.text_prev_name, self.text_ctc_name, self.text_word_name, self.text_name]
        if self.tokenizer is not None:
            for name in text_names:
                if name in data:
                    ...
                    if name == self.text_word_name:
                        # add word_time symbol to text word
                        text_ints = text_ints[1:]
                        if text_ints[0] != self.na_idx:
                            text_ints = np.insert(text_ints, 2, self.word_time_symbol)
                            text_ints[
                                np.logical_and(
                        ...
                        if self.train and np.random.uniform() > (self.utt_time_apply_prob + self.word_time_apply_prob):
                            # Remove timestamps
                        ...
                        elif np.random.uniform(0.0, self.utt_time_apply_prob + self.word_time_apply_prob) > self.utt_time_apply_prob and data[self.text_word_name][0] != self.na_idx:
                            # word-level timing, if available
                            # First two tokens are <category> and <task>
                            text_ints = np.insert(text_ints, 2, self.word_time_symbol)
                        else:
                            # utterance-level timing
                            text_ints = np.insert(text_ints, 2, self.utt_time_symbol)
        ...
        if self.text_name in data:
            if data[self.text_name][2] == self.word_time_symbol:
                data[self.text_name] = data[self.text_word_name]
