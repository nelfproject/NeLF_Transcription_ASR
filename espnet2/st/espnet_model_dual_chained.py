...
from espnet.nets.pytorch_backend.nets_utils import th_accuracy, mask_based_pad
...
class ESPnetSTModelDualChained(AbsESPnetModel):
    ...
    def __init__(
        ...
        if self.asr_weight > 0:
            ...
            if extra_asr_decoder is not None:
                logging.warning(
    ...
    def forward(
        ...
        if (self.asr_weight > 0 and self.mtlalpha < 1.0) or self.mt_weight > 0:
            mask_asr = src_text.sum(-1) > 0
            mask_mt = text.sum(-1) > 0
            (
                loss_asr_att,
                loss_mt_att,
                acc_asr_att,
    ...
    def _calc_dual_att_loss(
        ...
        ys_pad_asr: torch.Tensor,
        ys_pad_lens_asr: torch.Tensor,
        ys_pad_mt: torch.Tensor,
        ys_pad_lens_mt: torch.Tensor,
        ...
        ys_in_pad_mt, ys_out_pad_mt = add_sos_eos(ys_pad_mt, self.sos, self.eos, self.ignore_id)
        ys_in_lens_mt = ys_pad_lens_mt + 1

        # 1. Forward decoder
        decoder_out_mt, _, decoder_out_asr, _ = self.decoder(
            encoder_out, encoder_out_lens, ys_in_pad_asr, ys_in_lens_asr,
            ys_in_pad_mt, ys_in_lens_mt, mask_asr, mask_mt
        )

        ys_out_pad_combined = mask_based_pad(mask_asr, ys_out_pad_asr,
                                                       ys_out_pad_mt,
                                                       pad_val=self.ignore_id)

        if mask_asr.sum() == 0:
            loss_att_asr, acc_att_asr, cer_att_asr, wer_att_asr = 0.0, None, None, None
        else:
            decoder_out_asr_m = decoder_out_asr[mask_asr]
            ys_out_pad_asr_m = ys_out_pad_combined[mask_asr]
            ys_pad_asr_m = ys_pad_asr[mask_asr]

            loss_att_asr = self.criterion_asr(decoder_out_asr_m, ys_out_pad_asr_m)
            acc_att_asr = th_accuracy(
                decoder_out_asr_m.view(-1, self.src_vocab_size),
                ys_out_pad_asr_m,
                ignore_label=self.ignore_id,
            )

            if self.training or self.asr_error_calculator is None:
                cer_att_asr, wer_att_asr = None, None
            else:
                ys_hat_asr = decoder_out_asr_m.argmax(dim=-1)
                cer_att_asr, wer_att_asr = self.asr_error_calculator(ys_hat_asr.cpu(), ys_pad_asr_m.cpu())

        if mask_mt.sum() == 0:
            loss_att_mt, acc_att_mt, bleu_att = 0.0, None, None
        else:
            decoder_out_mt_m = decoder_out_mt[mask_mt]
            ys_out_pad_mt_m = ys_out_pad_combined[mask_mt]
            ys_pad_mt_m = ys_pad_mt[mask_mt]

            loss_att_mt = self.criterion_st(decoder_out_mt_m, ys_out_pad_mt_m)
            acc_att_mt = th_accuracy(
                decoder_out_mt_m.view(-1, self.vocab_size),
                ys_out_pad_mt_m,
                ignore_label=self.ignore_id,
            )
            ...
            if self.training or self.mt_error_calculator is None:
                ...
                ys_hat_mt = decoder_out_mt_m.argmax(dim=-1)
                bleu_att = self.mt_error_calculator(ys_hat_mt.cpu(), ys_pad_mt_m.cpu())

        return loss_att_asr, loss_att_mt, acc_att_asr, acc_att_mt, cer_att_asr, wer_att_asr, bleu_att
