...
from espnet2.asr.dual_decoder.transformer_dual_decoder import DualTransformerDecoder
...
from espnet2.st.espnet_model_dual import ESPnetSTModelDual
...
dual_decoder_choices = ClassChoices(
    "dual_decoder",
    classes=dict(
        transformer=DualTransformerDecoder
    ),
    type_check=AbsDualDecoder,
    default=None,
    optional=True
)
...
class STTask(AbsTask):
    ...
    class_choices_list = [
        ...
        decoder_choices,
        # --dual decoder overwrites decoder
        dual_decoder_choices,
    ...
    @classmethod
    def build_model(cls, args: argparse.Namespace) -> ESPnetSTModelDual:
        assert check_argument_types()
        ...
        # 5. Decoder
        if getattr(args, "dual_decoder", None) is not None:
            decoder_class = dual_decoder_choices.get_class(args.dual_decoder)

            decoder = decoder_class(
                vocab_size_src=src_vocab_size,
                vocab_size_tgt=vocab_size,
                encoder_output_size=encoder_output_size,
                **args.dual_decoder_conf,
            )
