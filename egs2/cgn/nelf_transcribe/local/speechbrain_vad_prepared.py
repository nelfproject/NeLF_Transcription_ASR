...
boundaries = list()
with open('/esat/audioslave/jponcele/NeLF/scoring/test_data/nbest/segments', 'r') as rd:
    line = rd.readline()
    while line:
        seg, fn, st, et = line.rstrip().split(' ')
        if fn == filename:
            boundaries.append([round(float(st), 3), round(float(et), 3)])
        line = rd.readline()
boundaries = torch.Tensor(boundaries)

boundaries, _ = torch.sort(boundaries, 0)
